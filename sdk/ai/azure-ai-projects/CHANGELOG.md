...
### Sample updates
* Added `project_client.agents.set_auto_toolcalls(toolset=toolset)` to all samples that has `toolcalls` executed by `azure-ai-project` SDK

### Bugs Fixed

### Breaking Changes
The `toolset` parameter in `create_agents` no longer executes `toolcalls` by default. Instead, calling `set_auto_toolcalls` now automatically executes `toolcalls` for `agents` with a `function tool` in `tools` or `toolset`.
